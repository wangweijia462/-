...
The continuous human-capital interaction is easier to see in
Figure~\ref{fig:marginal_hc}.
At low levels of pre-sample schooling the confidence band includes
zero, so suitability is not estimated precisely enough to imply a
clear reduction in inequality.
Once $\ln \hc_{70}$ rises above roughly 1.35, the 95 percent
confidence interval lies below zero; above the sample median,
the estimated marginal effect becomes steadily more negative.

%% ---- Figure 3: Marginal Effect of Suitability over HC ------
\begin{figure}[H]
\centering
\includegraphics[width=0.76\textwidth]{fig3_marginal_hc.pdf}
\caption{Marginal Effect of Suitability over the Human Capital
         Distribution}
\label{fig:marginal_hc}
\begin{minipage}{0.84\textwidth}
\footnotesize
\textit{Notes}: The solid line plots
$\partial\ln\gini_{it}/\partial\ln\suit_{it}$
implied by column~(3) of Table~\ref{tab:main}.
The shaded area gives the 95 percent confidence interval computed
from the full Driscoll--Kraay covariance matrix for
$\ln\suit$ and $\ln\suit\times\ln\hc_{70}$.
Vertical gray lines mark the 25th, 50th, and 75th percentiles of
pre-sample human capital.
\end{minipage}
\end{figure}
...
Figure~\ref{fig:binscatter} shows the baseline
suitability--inequality relationship after partialling out economy
fixed effects, year fixed effects, and the control vector.
The negative fitted slope in the binned scatterplot is not
driven by a small number of high-leverage observations;
the conditional relationship remains visible across the middle
of the residualized distribution.

%% ---- Figure 2: Conditional Binned Scatterplot ---------------
\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{fig2_binscatter.pdf}
\caption{Conditional Binned Scatterplot of Suitability and
         Inequality}
\label{fig:binscatter}
\begin{minipage}{0.84\textwidth}
\footnotesize
\textit{Notes}: The figure plots 20 quantile bins of residualized
$\ln\gini$ against residualized $\ln\suit$ after removing
economy fixed effects, year fixed effects, trade openness,
institutional quality, old-age dependency, and log population.
The fitted line is the Frisch--Waugh--Lovell slope from the
same partialled-out data.
\end{minipage}
\end{figure}
...
Figure~\ref{fig:lag_coefs} plots the three baseline lag
coefficients with 95 percent confidence intervals.
The visual pattern confirms what the table shows: the point
estimates barely move from $t-1$ to $t-3$, while precision
declines gradually as the lag length shortens the usable panel.

%% ---- Figure 4: Coefficient Plot for Lagged Specs -----------
\begin{figure}[H]
\centering
\includegraphics[width=0.64\textwidth]{fig4_lag_coefs.pdf}
\caption{Coefficient Plot for Lagged Suitability Specifications}
\label{fig:lag_coefs}
\begin{minipage}{0.84\textwidth}
\footnotesize
\textit{Notes}: Points report the coefficients on
$\ln\suit_{t-1}$, $\ln\suit_{t-2}$, and $\ln\suit_{t-3}$
from columns~(1)--(3) of Table~\ref{tab:lags}.
Error bars are 95 percent confidence intervals based on
Driscoll--Kraay standard errors.
$p$-values for the Driscoll--Kraay test of $H_0:\beta=0$
