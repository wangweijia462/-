...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, rounded corners=3pt, draw=black!70,
    fill=black!4, align=center, minimum height=0.7cm, text width=2.0cm,
    font=\footnotesize},
  mbox/.style={rectangle, rounded corners=3pt, draw=blue!60!black,
    fill=blue!8, align=center, minimum height=0.7cm, text width=2.0cm,
    font=\footnotesize},
  rbox/.style={rectangle, rounded corners=3pt, draw=green!50!black,
    fill=green!8, align=center, minimum height=0.7cm, text width=2.1cm,
    font=\footnotesize},
...
\node[box, below=0.8cm of info,  text width=2.2cm, font=\scriptsize,
  draw=gray!60, fill=gray!5]
  (t1) {Test: Table~\ref{tab:alpha_test}\\$\hat{a}_1=-0.016^{**}$};
\node[box, below=0.8cm of lpm,   text width=2.2cm, font=\scriptsize,
  draw=gray!60, fill=gray!5]
  (t2) {Mediation Step 1\\$\hat\beta_1=-0.214^{***}$};
\node[box, below=0.8cm of grad,  text width=2.2cm, font=\scriptsize,
  draw=gray!60, fill=gray!5]
  (t3) {Mediation Step 2\\$\hat\delta_1=+0.318^{***}$};
\node[box, below=0.8cm of frob,  text width=2.2cm, font=\scriptsize,
  draw=gray!60, fill=gray!5]
...
composite score from 0 to 100, with typical values in the 0--50 range. This database has been widely used in Chinese environmental
...
The full causal chain from EID to gradient consistency is not
empirically established. The mediation analysis accounts for 28\% of
the stability improvement through the reward-variance channel; the
remaining 72\% is plausibly attributable to direct informational
effects on state transitions, but testing this requires a
stronger identification strategy---ideally a difference-in-differences
design exploiting staggered mandatory disclosure adoption. Net-return
advantages persist at 0.1--0.3\% per-turnover costs but would erode
at higher levels not modeled here. All results are calibrated to
Chinese A-shares; whether similar EID-stability relationships hold in
